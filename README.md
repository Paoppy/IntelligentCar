...
├── aiCar/
│   ├── app.py           # 主程序入口（线程调度控制）
│   ├── car.om           # 目标检测模型（.om 格式，适用于昇腾等硬件）
│   ├── car.txt          # 模型识别类别标签文件
│   ├── ctrlCar.py       # 控制模块（向 Arduino 或电机控制模块发送指令）+ 车道偏移决策
│   ├── det_utils.py     # 目标检测的辅助函数
│   ├── model.py         # 模型加载与推理逻辑 + 车道线检测算法
│   └── templates/
│       └── web.html     # Flask 前端页面，用于实时视频流预览和交互控制
│
├── Arduino/
    └── Arduino.ino      # Arduino 控制代码（低层电机或传感器控制）
    
运行指令：
cd .../project/aicar  # 进入项目路径
（下面两条都可以）
python app.py         # 使用 python 直接运行
flask run --debug     # 使用 Flask CLI（命令行工具）启动应用，并开启调试模式。（可以自动重载）
```
...
## 备注：
本项目为课程实践，以上内容为部分内容介绍，详细信息可参考代码，欢迎交流和指正。
